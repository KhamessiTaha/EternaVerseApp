...
export const CodexPanel = ({ isOpen, onClose, universe }) => {
  ...
  const [principleId, setPrincipleId] = useState(CODEX_PRINCIPLES[0].id);
        ...
        <div className="flex items-center justify-between border-b border-line px-5 py-4">
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-ink-faint text-[10px] font-mono tracking-wider uppercase">
                {tab === 'catalog' ? 'Deep Field Catalog' : 'Principles of the Universe'}
              </p>
            </div>
          ...
          <div className="flex items-center gap-6 font-mono">
            ...
            {tab === 'catalog' ? (
              ...
              <div className="text-right">
                <div className="text-[9px] text-ink-faint uppercase tracking-wider">Principles</div>
                <div className="text-ink text-sm tabular-nums">{CODEX_PRINCIPLES.length}</div>
              </div>
            )}
            <button
              onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-6 font-mono">
              {selected && (
                <>
                  ...
                  {containment && (
                    <div className="mt-5 max-w-xl border-l-2 border-accent/50 pl-4">
                      <div className="text-[9px] font-mono uppercase tracking-[0.2em] text-accent mb-1.5">
                        Containment · the physics you use
                      </div>
                      <p className="text-ink-dim text-[13px] leading-relaxed">{containment}</p>
                    </div>
                  )}
                </>
              )}
            ...
            <div className="w-[340px] border-r border-line overflow-y-auto">
              {CODEX_PRINCIPLES.map((p) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 font-mono border-l-2 transition-colors ${
                    principle.id === p.id
                      ? 'border-accent bg-void-raised text-ink'
                      : 'border-transparent text-ink-dim hover:text-ink hover:bg-void-raised/50'
                  }`}
                >
                  <div className="text-[12px]">{p.title}</div>
                  <div className="text-[10px] text-ink-faint uppercase tracking-wider">{p.tag}</div>
                </button>
              ))}
            </div>

            <div className="flex-1 overflow-y-auto p-6 font-mono">
              <div className="text-ink-faint text-[11px] uppercase tracking-wider mb-1">{principle.tag}</div>
              <h3 className="font-sans text-2xl text-ink font-semibold mb-5">{principle.title}</h3>
              <p className="text-ink-dim text-[13px] leading-relaxed max-w-xl">{principle.body}</p>
              <div className="mt-6 max-w-xl border-t border-line pt-3">
                <div className="text-[9px] font-mono uppercase tracking-[0.2em] text-ink-faint mb-1">In this universe</div>
                <div className="text-[12px] text-ink-dim">{principle.appliesTo}</div>
              </div>
            </div>
          </div>
        )}
