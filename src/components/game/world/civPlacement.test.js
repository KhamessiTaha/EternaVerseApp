...
test("scale is derived from Kardashev type (planetary -> galactic)", () => {
  ...
  assert.equal(civScale("Type1"), "stellar");
  assert.equal(civScale("Type2"), "stellar");
...
test("host path lengthens with descent depth of the scale", () => {
  ...
  assert.equal(t1.length, 1);   // inside a galaxy
  assert.equal(t0.length, 2);   // inside a galaxy's star
  assert.equal(t1[0], t0[0]);   // same home galaxy across types
});
...
test("ascension: same civ shows at a higher scale when its type rises", () => {
  ...
  // it advances to Type I -> now visible at the stellar scale of its galaxy,
  // NOT at the old planetary system.
  const risen = { id: "civ_rise", type: "Type1" };
...
test("a civ is only visible in its own galaxy/system, not another", () => {
  const civ = { id: "civ_home", type: "Type1" };
